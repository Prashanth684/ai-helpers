...
# Allow override via environment variable, or try common relative locations
MAIN_AGENTIC="${OPENSHIFT_ENHANCEMENTS_AGENTIC:-}"
if [ -z "$MAIN_AGENTIC" ]; then
    # Try to find it relative to common locations
    for candidate in \
        "../enhancements/agentic" \
        "../../enhancements/agentic" \
        "../../../enhancements/agentic"; do
        if [ -d "$candidate" ]; then
            MAIN_AGENTIC="$candidate"
            break
        fi
    done
fi
